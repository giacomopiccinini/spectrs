...
fn test_save_spectrogram_image_all_colormaps() -> Result<()> {
    ...
    // Read audio
    let (audio, _sr) = read_audio_file_mono(audio_path.to_str().unwrap())?;
    ...
    for (colormap, filename) in colormaps {
        ...
        save_spectrogram_image(&spec, output_path.to_str().unwrap(), colormap)?;
...
fn test_save_spectrogram_image_viridis() -> Result<()> {
    ...
    save_spectrogram_image(&spec, output_path.to_str().unwrap(), Colormap::Viridis)?;
...
fn test_save_mel_spectrogram_image() -> Result<()> {
    ...
    // Read and process
    let (audio, sr) = read_audio_file_mono(audio_path.to_str().unwrap())?;
    ...
    save_spectrogram_image(
        &mel_spec,
        output_viridis.to_str().unwrap(),
        Colormap::Viridis,
    )?;
    save_spectrogram_image(&mel_spec, output_magma.to_str().unwrap(), Colormap::Magma)?;
...
fn test_save_spectrogram_image_edge_cases() -> Result<()> {
    ...
    save_spectrogram_image(&zero_spec, output_path.to_str().unwrap(), Colormap::Gray)?;
    assert!(output_path.exists());
...
fn test_save_spectrogram_different_dimensions() -> Result<()> {
    ...
    save_spectrogram_image(&wide_spec, output_path.to_str().unwrap(), Colormap::Inferno)?;
    assert!(output_path.exists());
    ...
    save_spectrogram_image(&tall_spec, output_path.to_str().unwrap(), Colormap::Viridis)?;
    assert!(output_path.exists());

    // Square spectrogram
    let square_spec = vec![vec![1.0; 128]; 128];
    let output_path = test_dir.join("square_spec.png");
    save_spectrogram_image(&square_spec, output_path.to_str().unwrap(), Colormap::Magma)?;
    assert!(output_path.exists());
...
fn test_complete_pipeline_with_image() -> Result<()> {
    ...
    save_spectrogram_image(&mel, output_path.to_str().unwrap(), Colormap::Viridis)?;
...
fn test_save_spectrogram_image_feature_disabled() {
    ...
    let result = save_spectrogram_image(&spec, "test.png", Colormap::Viridis);
